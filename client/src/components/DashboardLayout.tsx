    ...
    return (
      <div className="min-h-screen bg-[#F4F2ED] px-5 py-8 text-[#302A28]">
        <div className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-md flex-col items-center justify-center rounded-[2rem] border border-[#E9E1D9] bg-[#FFFDFC] p-10 text-center shadow-[0_24px_80px_rgba(68,46,37,0.08)]">
          <div className="atelier-stamp mb-7"><Sparkles className="h-5 w-5" /></div>
          <p className="font-serif text-sm tracking-[0.22em] text-[#BA5A4E]">ATELIER / SKILL</p>
          ...
          <Button onClick={() => startLogin()} className="mt-8 h-11 w-full rounded-xl bg-[#B95D50] text-white hover:bg-[#9F493D]">进入工作台</Button>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      <div ref={sidebarRef} className="relative">
        <Sidebar collapsible="icon" className="atelier-sidebar border-r border-[#333635] text-[#F6F1EA]" disableTransition={isResizing}>
          <SidebarHeader className="h-28 justify-center px-3">
            <div className="flex w-full items-center gap-3">
              ...
              {!isCollapsed && <div className="min-w-0"><div className="flex items-center gap-2"><span className="atelier-sidebar-mark"><Sparkles className="h-3.5 w-3.5" /></span><p className="atelier-wordmark">Skill Atelier</p></div><p className="mt-1 text-[10px] tracking-[0.18em] text-[#D9877C]">平行世界 · 内容运营</p></div>}
            </div>
          </SidebarHeader>
          <SidebarContent className="gap-0 px-2">
            ...
            <SidebarMenu>
              {menuItems.map(item => <SidebarMenuItem key={item.path}><SidebarMenuButton isActive={location === item.path} onClick={() => setLocation(item.path)} tooltip={item.label} className="h-11 rounded-xl px-3 text-[#D6D0C8] transition-all hover:bg-white/10 hover:text-white data-[active=true]:bg-[#B95D50] data-[active=true]:font-medium data-[active=true]:text-white"><item.icon className="h-4 w-4" /><span>{item.label}</span></SidebarMenuButton></SidebarMenuItem>)}
            </SidebarMenu>
          </SidebarContent>
      ...
      <SidebarInset className="bg-[#F4F2ED]">
        ...
        <main className="min-h-screen p-4 sm:p-6 lg:p-8">
          <div className="mb-5 hidden items-center justify-between rounded-2xl border border-[#E6E0D9] bg-[#FBFAF7]/80 px-5 py-3 backdrop-blur lg:flex">
            <div className="flex items-center gap-3"><span className="h-2 w-2 rounded-full bg-[#B95D50]" /><div><p className="text-xs font-medium tracking-[0.12em] text-[#A09288]">CURRENT DESK</p><p className="mt-0.5 text-sm font-semibold text-[#443A35]">{activeMenuItem?.label ?? "运营总览"}</p></div></div>
